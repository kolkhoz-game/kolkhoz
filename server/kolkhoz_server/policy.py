...
from engine.python.kolkhoz_c_engine import KCPolicyModelBuffer
...
class PolicyArtifact:
    ...
    def __post_init__(self) -> None:
        ...
        self._layer_weight_ptrs = (
            ctypes.POINTER(ctypes.c_double) * MAX_HIDDEN_LAYERS
        )()
    ...
    def head_count(self) -> int:
        return int(
            self.data.get(
                "head_count",
                self.data.get("headCount", len(self.data.get("b2s", [])) or 1),
            )
        )
    ...
    def scratch(
        ...
        *,
        ...
        if len(hidden_layers) > MAX_HIDDEN_LAYERS:
            raise ValueError(
                f"C policy backend supports at most {MAX_HIDDEN_LAYERS} hidden layers"
            )
    ...
    def c_buffer(self) -> KCPolicyModelBuffer:
        if self.backend != "c-mlp":
            raise ValueError(
                f"policy backend {self.backend!r} cannot be loaded into the C MLP backend"
            )
        self._arrays.clear()
        ...
        if layer_count and hidden_weights and hidden_biases:
            for index in range(layer_count):
                ...
                self._layer_weight_ptrs[index] = ctypes.cast(
                    weights, ctypes.POINTER(ctypes.c_double)
                )
                self._layer_bias_ptrs[index] = ctypes.cast(
            ...
            output = _double_array(
                _flat(self.data.get("output_weights", self.data.get("outputWeights")))
            )
            ...
            w2 = _double_array(
                _flat(
                    self.data.get(
                        "w2",
                        self.data.get("output_weights", self.data.get("outputWeights")),
                    )
                )
            )
    ...
    def sync_from_c(self) -> None:
        ...
        if layer_sizes:
            ...
            self.data["output_weights"] = [
                float(self._output_array[i])
                for i in range(layer_sizes[-1] * head_count)
            ]
        else:
            ...
            self.data["w1"] = [
                float(w1[i]) for i in range(self.input_size * hidden_size)
            ]
